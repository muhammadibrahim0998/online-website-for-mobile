...
const Vivo = () => {
  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/products?category=vivo");
      setProducts(res.data);
    } catch (err) {
      console.error("Error fetching Vivo:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container py-5">
      <h2 className="fw-bold mb-4" style={{ color: "#00b4d8" }}>Vivo Smartphones</h2>
      <div className="row g-4">
        ...
        {products.length === 0 && <div className="col-12 text-center text-muted py-5">No Vivo phones available in store yet.</div>}
      </div>
    </div>
  );
};
